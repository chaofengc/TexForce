...
def get_root_logger(logger_name='texforce', log_level=logging.INFO, is_main_process=True):
...
class DDPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.logger = get_root_logger(is_main_process=self.accelerator.is_main_process)
        ...
        self.reward_fn.to(self.accelerator.device, dtype=inference_dtype)
        ...
        T_max=self.config.num_epochs * self.config.train_num_inner_epochs * self.config.sample_num_batches_per_epoch // self.config.train_gradient_accumulation_steps
        self.scheduler = LinearWarmupCosineAnnealingLR(
            self.optimizer,
            warmup_epochs=int(0.03 * T_max),
            max_epochs=T_max,
        )
        ...
        if hasattr(self.sd_pipeline, "use_lora") and self.sd_pipeline.use_lora:
            unet, text_encoder, self.optimizer, self.scheduler = self.accelerator.prepare(self.sd_pipeline.unet, self.sd_pipeline.text_encoder, self.optimizer, self.scheduler)
            self.trainable_layers = list(filter(lambda p: p.requires_grad, unet.parameters())) + list(filter(lambda p: p.requires_grad, text_encoder.parameters()))
        else:
            self.trainable_layers, self.optimizer = self.accelerator.prepare(trainable_layers, self.optimizer)
        
        for n, p in self.sd_pipeline.unet.named_parameters():
            if p.requires_grad:
                self.logger.info(f"Training: {n} with shape {p.shape}")
        for n, p in self.sd_pipeline.text_encoder.named_parameters():
            if p.requires_grad:
                self.logger.info(f"Training: {n} with shape {p.shape}")
            
        self.logger.info(f"Trainable param numbers: {sum(p.numel() for p in self.trainable_layers) / 1e6}M")

        if self.config.async_reward_computation:
            self.executor = futures.ThreadPoolExecutor(max_workers=config.max_workers)

        if config.resume_from:
            self.logger.info(f"Resuming from {config.resume_from}")
    ...
    def step(self, epoch: int, global_step: int):
        ...
        self.logger.info(f"Sampling images for training. Global epoch: {epoch}\tBatch num: {self.config.sample_num_batches_per_epoch}\tBatch size: {self.config.sample_batch_size}\tProcesses: {self.accelerator.num_processes}\tTotal samples per epoch: {samples_per_epoch}\tTrain batch size: {total_train_batch_size}")
        ...
        if epoch % self.config.save_freq == 0:
            ...
            output_dir = f"{self.accelerator.project_dir}/checkpoints/checkpoint_{self.accelerator.save_iteration}"
            self.logger.info(f"Saving model at epoch {epoch} to {output_dir}")
    ...
    def _generate_samples(self, iterations, batch_size):
        ...
        for _ in tqdm(range(iterations), disable=not self.accelerator.is_local_main_process):
    ...
    def _train_batched_samples(self, inner_epoch, epoch, global_step, batched_samples):
        ...
        for _i, sample in enumerate(batched_samples):
            ...
            avg_timestep_losses = torch.zeros(3, self.num_train_timesteps, device=self.accelerator.device)
            ...
            for j in range(self.num_train_timesteps):
                ...
                with self.accelerator.accumulate([self.sd_pipeline.unet, self.sd_pipeline.text_encoder]):
                    ...
                    avg_timestep_losses[0, j] = loss
                    avg_timestep_losses[1, j] = approx_kl
                    avg_timestep_losses[2, j] = clipfrac
            ...
            self.logger.info(f"Global epoch: {epoch}/{self.config.num_epochs} Global step: {global_step} Inner epoch: {inner_epoch}/{self.config.train_num_inner_epochs} Inner step: {_i}/{len(batched_samples)}\tLR: {self.scheduler.get_last_lr()[0]:.6f}\tLoss: {avg_timestep_losses[0].mean():.4f}\tApprox KL: {avg_timestep_losses[1].mean():.4f}\tClipfrac: {avg_timestep_losses[2].mean():.4f}")
