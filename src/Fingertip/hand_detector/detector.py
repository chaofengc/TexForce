...
class YOLO:
    def __init__(self, weights, threshold=0.3):
        self.f = yoloFlag()
        self.model = yolo_model()
        self.threshold = threshold
        self.model.load_weights(weights)
    
    def detect_prob(self, image):
        """
        input: (B, C, H, W), [0, 1], RGB, Pytorch Tensor 
        """
        image = torch.nn.functional.interpolate(image, (self.f.target_size, self.f.target_size))
        image = image.permute(0, 2, 3, 1).numpy()
        assert image.shape[1] == self.f.target_size and image.shape[2] == self.f.target_size, f'input size should be {self.f.target_size}, but got {image.shape}'
        assert image.shape[3] == 3, 'input should be (B, H, W, 3)'
        ...
        scores = np.amax(grid_pred, axis=(1, 2))
        return scores
